...
from .utils.data_objects import RoundData, CertAuth
...
class Round(BaseModel):
  ...
  def execute(self):
    ...
    round_data = RoundData(
        ...
        ca_names=[ca.name for ca in self.cas],
        ip_announced=self.ip_address,
        turns=[]
    )
    args = ["-d", self.node_a.name, self.node_b.name, "-i", self.ip_address]
    ...
    for ca in self.cas:
      turn = TurnFactory.create(ca.name, ca.url, self.node_a, self.node_b)
